    ...
    <hr class="m-0" style="color:#86a2ae;" />
      ...
      <div class="d-flex flex-column mb-3" >
        <div class="d-flex align-items-center mt-2">
          ...
          <span class="author">{{ group.title }}</span>
          <span style="color: #86a2ae" class="m-1">•</span>
          <span class="date">{{ props.group.lastMessage.time? getTimeAgo(props.group.lastMessage.time):''   }}</span>
        </div>
        <p class="ms-2 " style="color: #b3cad5; font-size: 0.95em;">
          {{ props.group.lastMessage? ((group.lastMessage.sender == auth.currentUser.uid ? 'You:': '')) + group.lastMessage.content : '' }}
        </p>
      </div>
